...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const getUtmUrl = (baseUrl: string, category: string) => {
    // Remove acentos e espaços para a UTM ficar limpa
    ...
    // Verifica se o link já tem interrogação para não quebrar a URL
  ...
  return (
    ...
    >
      {/* SELO DE MAIS VENDIDO / PRODUTO DO DIA */}
      {product.isDailyTip && (
      ...
      <div className="p-5 space-y-3">
        <h3 className="text-sm font-bold text-slate-800 line-clamp-2 h-10 leading-snug group-hover:text-blue-600 transition-colors">
          {product.name}
        </h3>
        ...
        {/* BOTÃO DE AÇÃO COM LINK UTM */}
        <a
          href={finalUrl}
          target="_blank"
          rel="noopener noreferrer"
          className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl font-extrabold text-xs uppercase tracking-widest transition-all active:scale-95
            ${product.isDailyTip 
